...
from gemini_api import call_gemini_api_for_evaluation
...
def register_routes(app):
            ...
            if request.method == 'POST':
                ...
                if file and app.config['ALLOWED_EXTENSIONS'] and file.filename.rsplit('.', 1)[1].lower() in app.config['ALLOWED_EXTENSIONS'] \
                   and reference_file and app.config['ALLOWED_EXTENSIONS'] and reference_file.filename.rsplit('.', 1)[1].lower() in app.config['ALLOWED_EXTENSIONS']:
                    ...
                    # New: Send notification for new assignment
        ...
        def grade_submission(submission_id):
            ...
            try:
                ...
                reference_images = convert_from_path(assignment_doc['reference_file_path'])
    
                all_images = student_images + reference_images
                
                prompt = f"""
                You are an AI grading assistant. Your task is to evaluate a student's answer against a reference answer.
                
                The first set of images are the student's submission. The second set of images are the teacher's reference answer key.
                
                Based on the provided documents, give a score from 0 to 100 for the student's work and provide constructive feedback of 1-2 sentences.
                ...
                if gemini_response:
                    ...
                    # New: Send notification for evaluation complete
        ...
        def reminders():
            """
            New endpoint for n8n to call on a schedule.
            Checks for assignments due in the next 24 hours.
            """
            if app.db is None:
            ...
            assignments_due_tomorrow = list(app.db.assignments.find({
                'due_date': {'$gte': today.strftime('%Y-%m-%d'), '$lt': tomorrow.strftime('%Y-%m-%d')}
            }))
            
            for assignment in assignments_due_tomorrow:
                teacher_doc = app.db.users.find_one({'_id': ObjectId(assignment['teacher_id'])})
                if teacher_doc:
                    reminders.append({
